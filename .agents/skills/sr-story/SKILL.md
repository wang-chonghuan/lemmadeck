...
5. `capability-5-persist`: **persist** a chapter (Markdown → `md` column) + its questions into `lemmadeck-schema.sr_stories` / `sr_story_chapters` / `sr_story_questions`. → `references/capability-5-persist/persist.md`, `scripts/save-story.mjs`.
