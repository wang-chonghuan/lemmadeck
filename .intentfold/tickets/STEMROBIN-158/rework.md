...
- 第一轮 FigureSpec validator 26 项通过。新增正例验证 9×7 网格和四个位移；反例分别证明
...
- Plane 评论 `e84eedc9-5194-49f8-a211-3a7ff7455703` 确认原图样本和实际字号修复
  已通过，但指出两个门禁仍会漏放行：`pointRelationships` 只检查同组有任意关系，
  未保证每个来源点被覆盖；正式 `check_product.mjs` 的说明宣称检查实际字号，实现却
  只检查媒体非空、像素和滚动。
- validator 现只统计点引用完全位于当前来源组的多点断言，并要求每个映射点至少被
  一项此类断言覆盖。五点样本完整四个位移通过；删除 `O→M` 后，即使其他三个位移仍在，
  `M` 也会因缺少覆盖失败；把 `M` 连接到组外点同样不能替代。
- 正式 `check_product.mjs` 现读取图片 manifest 指向的 FigureSpec
  `display.minTextPx`，用每个 SVG 文字节点的 `getScreenCTM()` 计算最终屏幕字号并执行
  下限断言。持久 Playwright 回归使用同一 SVG 验证 640px / 20px 通过、CSS 缩至
  256px / 8px 时失败，不再依赖工单临时报告才能发现该问题。
- 第二轮复验通过：validator 29 项、图片 renderer 5 项、正式产品检查持久测试，以及
  审查者原六项脚本全部通过。六项脚本现在会拒绝漏掉 `O→M` 的来源组和 8px SVG，
  同时接受完整来源组与 20px SVG。
