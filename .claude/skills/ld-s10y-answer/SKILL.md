...
- `exact`：MathLive `.value` 的 LaTeX 与 `expected` 中任一项经过共享
  `app/src/lib/answer-normalize.ts` 规范化后必须匹配；规范化会统一受支持的 Unicode/LaTeX
  符号和输入包装，但保留分式分子、分母及外部乘法的作用域。作者不需要枚举纯序列化差异，
  也不得把数学结构不同的式子列为同一答案。
- `numeric`：数值等价。
- `expression`：数学表达式等价。
...
MathLive 元素，执行一次保持数学内容不变的真实键盘编辑，再读取编辑器重新序列化后的
`.value` 并调用产品共享规范化器；未触发输入事件、编辑后数学内容变化、判分不一致都会失败。
每个候选使用独立文档；浏览器焦点事务未完成时最多重试三次，并在报告中记录尝试次数。
这验证的是实际编辑后的序列化合同，不是 setter 原串缓存。公共正反例在
...
4. 全部小问 `judge=exact` → `math`；其 expected 已由 cap2 的 MathLive 实际编辑序列化
   gate 验证，不得再用 `needsAuthoring` 代替判分可靠性
...
> 小问数 = 2 × 具名点数，且**逐个小问**与「某个具名点的某条轴」有序对应：
> 第 n 个小问的 `expected` 必须就是它所声明的那个坐标。

对应关系写进规格的 `parts`（`{"point":"M","axis":"x","label":"M 向右","unit":"格"}`），
消费方不必再假设任何顺序 —— 只验成员关系是不够的，那样消费方仍得自己假设「第 2i 个小问是
第 i 个点的横坐标」，而消灭这类假设正是规格这一层存在的理由。
