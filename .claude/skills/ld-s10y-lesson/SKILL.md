...
- 现代 edition 的正文由工具按完整语句和单独引出的公式确定性分行；完整的数字、拉丁字母
  或西里尔字母分题序列必须统一成自然顺序，且题干前缀和每个分题各自独占一行。
  `adapt-finalize` 会重写已有 edition，不允许连续大段正文、错序或横向分题通过
...
4. 原始层继续按印刷视觉行忠实保存；现代版正文按完整语句和单独引出的公式分行。
   若一道题含完整的数字、拉丁字母或西里尔字母小问序列，必须按自然顺序排列，
